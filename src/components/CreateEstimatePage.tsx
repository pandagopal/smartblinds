...
const CreateEstimatePage: React.FC = () => {
  ...
  const generateEstimate = () => {
    // Mock estimate data generation
    const mockItems = windows.map(window => {
      // Calculate mock price based on dimensions and product type
      const width = parseFloat(window.width) || 30;
      const height = parseFloat(window.height) || 60;
      const sqFt = (width * height) / 144; // Convert to square feet

      let basePrice = 0;
      switch(window.productType) {
        case 'Cellular Shades':
          basePrice = 65;
          break;
        case 'Wood Blinds':
          basePrice = 85;
      ...
      const price = Math.round(basePrice * sqFt * 100) / 100;
