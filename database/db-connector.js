...
const categoryQueries = {
  ...
  getCategoryByIdOrSlug: async (identifier, isSlug = false) => {
    ...
    const query = `SELECT * FROM categories WHERE ${idField} = $1 AND is_active = TRUE`;
...
const cartQueries = {
  ...
  getCart: async ({ userId, sessionId }) => {
    ...
    if (userId) {
      cartQuery = 'SELECT * FROM carts WHERE user_id = $1 AND is_active = TRUE LIMIT 1';
      params = [userId];
    } else if (sessionId) {
      cartQuery = 'SELECT * FROM carts WHERE session_id = $1 AND is_active = TRUE LIMIT 1';
      params = [sessionId];
    } else {
    ...
    try {
      ...
      if (cartResult.rows.length === 0) {
        ...
        if (userId) {
          cartResult = await pool.query(
            'INSERT INTO carts (user_id, is_active) VALUES ($1, TRUE) RETURNING *',
            [userId]
          );
        } else {
          cartResult = await pool.query(
            'INSERT INTO carts (session_id, is_active) VALUES ($1, TRUE) RETURNING *',
            [sessionId]
          );
        }
      }
      ...
      const itemsQuery = `
        ...
        LEFT JOIN materials m ON ci.material_id = m.material_id
        WHERE ci.cart_id = $1
      `;
